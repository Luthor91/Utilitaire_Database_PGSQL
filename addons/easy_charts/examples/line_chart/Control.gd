...
#	x1 = ArrayOperations.multiply_float(range(-10, 11, 1), 0.5)
#	x2 = ArrayOperations.multiply_float(range(-10, 11, 1), 0.5)
	x = ArrayOperations.multiply_float(range(-10, 11, 1), 0.5)
	# And our y values. It can be an n-size array of arrays.
	# NOTE: `x.size() == y.size()` or `x.size() == y[n].size()`
#	y1 = ArrayOperations.multiply_int(ArrayOperations.cos(x1), 20)
#	y2 = ArrayOperations.multiply_int(ArrayOperations.cos(x2), 20)
	y = ArrayOperations.multiply_int(ArrayOperations.cos(x), 20)
...
#	var y3: Array = ArrayOperations.add_float(ArrayOperations.multiply_int(ArrayOperations.cos(x), -5), -3)
	...
	f2 = Function.new(x, y, "Humidity", { color = Color("#ff6384"), type = Function.Type.LINE, marker = Function.Marker.CROSS, interpolation = Function.Interpolation.SPLINE })
